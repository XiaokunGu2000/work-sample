...
def path(all_img_path):
    ...
    for path in all_img_path:
        ...
        mat = np.array(cv2.resize(imgg,(32,32)))
        # 可改成64 64 学习率降低来实现更广的分类
        train.append(mat.flatten())
    return train
...
def stocGradAscent3(w, Epoch=10,epsilon = 10 ** (-7)):
    gradient = grad(sigmoid)
    hess = hessian(sigmoid)
    weight = [w]
    cost = [sigmoid(w)]
    for i in range(1, Epoch + 1):
        gradtemp = gradient(w)
        hesstemp = hess(w)
        hesstemp.shape = (int((np.size(hesstemp)) ** (0.5)), int((np.size(hesstemp)) ** (0.5)))
        A = hesstemp + epsilon * np.eye(w.size)
        b = gradtemp
...
def DCTest(train, test, testL):
    original = np.random.rand(train.shape[0] + 1, 1)*0.012
    trainWs = stocGradAscent3(original, 5)
